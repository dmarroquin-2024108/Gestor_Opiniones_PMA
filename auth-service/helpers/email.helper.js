    ...
    const mailOptions = {
        ...
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <h2 style="color: #333;">¡Bienvenido a Gestión Corp, ${firstName}!</h2>
                <p>Tu cuenta ha sido creada exitosamente. Para activarla y poder acceder a todos los servicios, haz clic en el siguiente enlace:</p>
...
export const sendWelcomeEmail = async (email, firstName, username) => {
    const mailOptions = {
        from: process.env.EMAIL_USER,
        to: email,
        subject: 'Bienvenido a Gestor de Opiniones - Tu cuenta ha sido creada',
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <h2 style="color: #333;">¡Hola ${firstName}!</h2>
                <p>Tu cuenta en Gestor de opiniones ha sido creada exitosamente por un administrador.</p>
                
                <div style="background-color: #f5f5f5; padding: 20px; border-radius: 5px; margin: 20px 0;">
                    <h3 style="margin-top: 0;">Tu nombre de usuario:</h3>
                    <p><strong>${username}</strong></p>
                    <p style="color: #666;">Tu administrador te proporcionará tu contraseña de acceso.</p>
                </div>
                
                <p>Recibirás otro correo con un enlace para activar tu cuenta. Una vez activada, podrás iniciar sesión con las credenciales que te proporcione tu administrador.</p>
...
export const sendPasswordChangedEmail = async (email, firstName) => {
    const mailOptions = {
        from: process.env.EMAIL_USER,
        to: email,
        subject: 'Tu contraseña ha sido cambiada - Gestión de Opiniones',
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <h2 style="color: #333;">Hola ${firstName},</h2>
...
export const sendUserEditedEmail = async (email, firstName)=>{
    const mailOptions = {
        from: process.env.EMAIL_USER,
        to: email,
        subject: 'Usuario editado exitosamente - Gestión de Opiniones',
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <h2 style="color: #333;">Hola ${firstName},</h2>
                <p>Te confirmamos que tu usuario se ha editado exitosamente.</p>
                
                <div style="background-color: #d4edda; border-left: 4px solid #28a745; padding: 15px; margin: 20px 0;">
                    <p style="margin: 0; color: #155724;">Tu usuario se actualizó el día ${new Date().toLocaleString('es-ES')}</p>
                </div>
